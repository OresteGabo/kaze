# Terms of Use

Last updated: April 4, 2026

## Overview

These Terms of Use describe the rules for access to and use of Kaze.

Kaze is a private hospitality software product developed by GABO.

Contact:
- See [CONTACT.md](CONTACT.md)

## Acceptance

By accessing, testing, deploying, or using Kaze, you agree to these Terms of Use and any separate written commercial agreement that may apply.

## Authorized Use Only

Kaze may only be used by:

- authorized business customers
- authorized hotel or venue staff
- approved internal team members
- approved contractors or operators
- end users using the service through an authorized business deployment

## Prohibited Conduct

You may not:

- reverse engineer the product except where law explicitly allows it
- copy or redistribute the product without authorization
- attempt to bypass access controls
- misuse hotel, guest, or venue data
- interfere with the platform or related infrastructure
- use the product for unlawful conduct

## Intellectual Property

Kaze and all related materials remain the intellectual property of GABO unless otherwise provided by written agreement.

## Service Availability

Kaze may change over time. Features may be added, removed, limited, or refined as the product evolves.

## Business Integrations

Some features may depend on third-party systems such as hotel property systems, notifications, mapping assets, or venue operations platforms. Availability of those features may depend on external integrations.

## Privacy

Use of the product may involve personal data and operational data. See:
- [PRIVACY_POLICY.md](PRIVACY_POLICY.md)

## Disclaimer

Unless otherwise agreed in writing, Kaze is provided on an "as is" and "as available" basis during development, pilot, and pre-production stages.

## Limitation

To the maximum extent allowed by law and absent a separate written contract, GABO is not liable for indirect, incidental, special, or consequential damages arising from use of the product.

## Governing Agreements

If a signed commercial agreement exists, that agreement may govern over these general terms where applicable.

## Contact

- See [CONTACT.md](CONTACT.md)
